...
var process Process
var gameModule uintptr

func speedhack() {

	process, _ = ProcessByName("Among Us")
	log.Printf("Base: 0x%06X", process.ModBaseAddr)

	gameModule = process.Modules["GameAssembly.dll"].ModBaseAddr
	log.Printf("Client Module Base: 0x%06X", gameModule)

	A_ptr := gameModule + 0x01C57F7C
	A, _ := process.ReadInt(A_ptr)
	B_ptr := uintptr(A) + 0x5C
	B, _ := process.ReadInt(B_ptr)
	C_ptr := uintptr(B) + 0x4
	C, _ := process.ReadInt(C_ptr)
	speed_ptr := uintptr(C) + 0x14
	// READ
	speed, _ := process.ReadFloat32(speed_ptr)
	//fmt.Println(speed)

	fmt.Println("Entrez la vitesse désirée : ")
	var vitesse float32
	fmt.Scanf("%f", &vitesse)
	fmt.Println(vitesse)
	err := process.WriteFloat(speed_ptr, vitesse)
	if err != nil {
		fmt.Println(err)
	}
...
func cvision() {
	...
	gameModule = process.Modules["GameAssembly.dll"].ModBaseAddr
	log.Printf("DLL Module Base: 0x%06X", gameModule)
	...
	C_ptr := uintptr(B) + 0x4
	C, _ := process.ReadInt(C_ptr)
	cvision_ptr := uintptr(C) + 0x18
	// READ
	cvision, _ := process.ReadFloat32(cvision_ptr)
	//fmt.Println(cvision)
	// WRITE
	_ = process.WriteFloat(cvision_ptr, float32(0.5))
